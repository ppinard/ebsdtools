...
#    self.fail("Test if the testcase is working.")
  ...
  def testCalculateOrientation(self):
    ...
    for input in inputs:
      ...
      angles = eulers.negativeEulers(eulerAngles[0]/180.0*pi, eulerAngles[1]/180.0*pi, eulerAngles[2]/180.0*pi) 
      
      qSpecimenRotation = quaternions.quaternion(1,0,0,0)
      qCrystalRotation = quaternions.eulerAnglesToQuaternion(angles)
      print qCrystalRotation, eulers.degEulers(eulers.positiveEulers(qCrystalRotation.toEulerAngles()))
...
#      q = orientation.calculateOrientation(n1, n2, vectors.vector(planes['111']['vector']), vectors.vector(planes['1-1-1']['vector']))
      ...
      qf =   q
      print eulers.degEulers(eulers.positiveEulers(qf.toEulerAngles()))
